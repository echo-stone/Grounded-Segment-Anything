...
def mask_to_polygon(mask: np.ndarray, tolerance: float = 0.5) -> List[List[int]]:
    """
    마스크를 회전된 직사각형의 꼭지점 좌표로 변환합니다.

    Args:
        mask: 2D numpy array (binary mask)
        tolerance: 사용되지 않음 (minAreaRect에서는 불필요)

    Returns:
        List of [x,y] coordinates representing the rotated rectangle vertices
    """
    try:
        ...
        # Convert box points to list of [x,y] coordinates
...
async def analyze_image_with_visualization(
        ...
        box_threshold: float = Form(0.3),
        text_threshold: float = Form(0.25),
        tolerance: float = Form(0.01)  # This parameter is kept for API compatibility but not used
) -> FileResponse:
    try:
        # 입력 이미지 저장
        image_path = os.path.join(OUTPUT_DIR, "input_image.jpg")
        ...
        for mask_idx, (mask, box, phrase, color) in enumerate(zip(masks, boxes_filt, pred_phrases, colors)):
            ...
            # 4. 최소 면적 직사각형
            rect_img = np.ones_like(mask_binary) * 255
            if contours:
                main_contour = max(contours, key=cv2.contourArea)
                rect = cv2.minAreaRect(main_contour)
                box = cv2.boxPoints(rect)
                box = np.int0(box)
                cv2.drawContours(rect_img, [box], 0, 0, 2)

                ax4 = plt.subplot(num_masks, num_stages, mask_idx * num_stages + 4)
                ax4.imshow(rect_img, cmap='gray', vmin=0, vmax=255)
                ax4.set_title(f'Minimum Area Rectangle {mask_idx + 1}\nAngle: {rect[2]:.1f}°')
                ax4.axis('off')
            ...
            polygon = mask_to_polygon(mask_binary)
            if polygon:
